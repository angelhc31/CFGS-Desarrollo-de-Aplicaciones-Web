...
if (isset($_SESSION["nombre"])) {
    if ($_SESSION["activo"]) {
        ...
        $_SESSION["filtroNombre"] = $_POST["nombre"];
        $_SESSION["filtroFecha"] = $_POST["fecha"];
        $_SESSION["filtroTurno"] = $_POST["turno"];
        $_SESSION["filtroEstado"] = $_POST["estado"];

        $consulta = "SELECT * FROM reservas WHERE negocio=".$_SESSION["idNegocio"];

        if ($_POST["nombre"] != "") {
            $consulta .= " AND nombreCliente LIKE '".mb_strtoupper(trim($_POST["nombre"]), 'UTF-8')."%'";
        }

        if ($_POST["fecha"] != "") {
            $consulta .= " AND fecha = '".$_POST["fecha"]."'";
        }

        if ($_POST["turno"] != "") {
            $consulta .= " AND turno = '".$_POST["turno"]."'";
        }

        if ($_POST["estado"] != "") {
            $consulta .= " AND estado = '".$_POST["estado"]."'";
        }
